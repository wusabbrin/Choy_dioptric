...
def main_with_cxn(
    ...
    # seq_args = [100, readout, 'laserglow_532', -1]
    ...
    for run_ind in range(num_runs):
        ...
        # laser_msquared.start_table_tuning(736.5,23) 
        # Take a sample and step through the shuffled frequencies
        # shuffle(freq_ind_list)
        for freq_ind, res_prec in enumerate(freqs):
            print("freq_ind: {}, res_prec: {}".format(freq_ind, res_prec))
            # laser_msquared.etalon_status(27)
            # laser_msquared.start_table_tuning()
            laser_msquared.tune_resonator(res_prec,651)         
            # Break out of the while if the user says stop
    ...
    # conversion_factor = (0.00001 / 3) * 1e6  # MHz per res_prec unit
    conversion_factor = 260  # MHz per res_prec unit updated 3/5
    
    freqs_MHz = [freq * conversion_factor for freq in freqs]
    print("freqs in MHz: ", freqs_MHz)
    ...
    ax.set_title('Signal Counts vs. Frequency (MHz)')
    ...
    data = {
        ...
        "readout": readout,
        "readout-units": "ns",
        "freq_index_master_list": freqs_MHz,
        # "opti_coords_list": opti_coords_list,
        # "opti_coords_list-units": "V",
